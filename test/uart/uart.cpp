    ...
    ui(new Ui::Uart)
{
    ...
    ui->pushButton_232->setVisible(false);
    ui->pushButton_422->setVisible(false);
    ui->pushButton_485->setVisible(false);
    ui->pushButton_loop->setVisible(false);
    //接收定时器
...
{
    if(-1 == ioctl(fd_com_mode_sel, TEST_LOOPBACK_CMD_APLEX, 0))
    {
        printf("loop back set fail\n");
        return;
    }
}
...
void Uart::on_pushButton_232_clicked()
{
    ...
    if(-1 == ioctl(fd_com_mode_sel, TEST_RS232_CMD_APLEX, 0))
    {
        printf("rs232 set fail\n");
        return;
    }
}
...
{
    if(-1 == ioctl(fd_com_mode_sel, TEST_RS485_CMD_APLEX, 0))
    {
        printf("rs485 set fail\n");
        return;
    }
}
...
{
    if(-1 == ioctl(fd_com_mode_sel, TEST_RS422_CMD_APLEX, 0))
    {
        printf("rs422 set fail\n");
        return;
    }
}
